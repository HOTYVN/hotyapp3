...
class KinCommentModify extends StatefulWidget {
  ...
  const KinCommentModify({Key? key,
  ...
  @override
  State<KinCommentModify> createState() => _KinCommentModifyState();
}

class _KinCommentModifyState extends State<KinCommentModify> {
    ...
    initview().then((_) {
      setState(() {
       ...
       // _selectedValue = view["data"]["main_category"];
        ...
        _isChecked == (view["data"]["public_yn"] == "N" ? true : false);
        filelist = view["files"];
      });
    });
                ...
                Container(
                  ...
                  child : TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      contentPadding : EdgeInsets.fromLTRB(15 * (MediaQuery.of(context).size.width / 360), 5 * (MediaQuery.of(context).size.height / 360), 0, 0),
                      // labelText: 'Search',
                      hintText: "\n궁금한 내용을 질문해주세요.\n\n질문 시 이런점을 주의해 주세요!\n답변이 등록되면 질문 수정/삭제가 불가합니다.\n질문 내용에 개인정보(실명, 전화번호, 메신저, 주소)가 포함되지 않게 해 주세요.",
  ...
  Future<void> FlutterDialog(context) async {
      ...
      if (res["result"] != null && res["resultstatus"] == "N") {
        Navigator.push(context, MaterialPageRoute(
          builder: (context) {
            return KinCommentList(article_seq: widget.article_seq, head_seq: widget.head_seq, parent_seq: widget.parent_seq, table_nm: widget.table_nm, adopt_chk: '', state: widget.state);
          },
        ));
      }
    }
