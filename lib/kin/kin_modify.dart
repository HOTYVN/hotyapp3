...
class KinModify extends StatefulWidget {
  final int article_seq;
  final dynamic table_nm;


  const KinModify({Key? key,
    required this.article_seq,
    required this.table_nm
  }) : super(key:key);

  @override
  State<KinModify> createState() => _KinModifyState();
}

class _KinModifyState extends State<KinModify> {
  ...
  Future<void> FlutterDialog(context) async {
    ...
    if(int.parse(_pointController.text) < 100) {
      ...
      final res = await dio.post(
          "http://www.hoty.company/mf/community/modify.do", data: _formData)
          .then((res) {
        return res.data;
      });
      ...
      if (res["result"] != null && res["resultstatus"] == "N") {
        showDialog(
          ...
          if(value == "view") {
            Navigator.push(context, MaterialPageRoute(
              builder: (context) {
                return KinView(article_seq: int.parse("${view['data']['article_seq']}"), table_nm: 'KIN', adopt_chk: '');
              },
            ));
          }
        });
